...
第二种是<font color="red"><code>===</code></font>比较，它不会自动转换数据类型，如果数据类型不一致，返回false，如果一致，再比较。
...
在同一个页面的不同的JavaScript文件中，如果都不用<font color="red"><code>var</code></font>申明，恰好都使用了变量i，将造成变量i互相影响，产生难以调试的错误结果。
